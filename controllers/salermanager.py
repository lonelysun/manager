...
#         return json.dumps(val,sort_keys=True)
    ...
    #获取管理首页数据
    @http.route('/manager/salepanel',type="http",auth="none",csrf=False)
    ...
    #获取任务列表
    @http.route('/manager/finishtracklist', type='http', auth="none",csrf=False)
    ...
    #获取团队人员列表
    @http.route('/manager/teamsaler', type='http', auth="none",csrf=False)
    def teamsaler(self,**post):
        ...
        hr_ids= user_obj.search(request.cr, SUPERUSER_ID,domain, int(indexPage),10,context=request.context)
        print('+++++++++++++++++++++++++++++')
        print(hr_ids)
        hrs= user_obj.browse(request.cr, SUPERUSER_ID,hr_ids)
        ...
        for hr_id in hrs:
            ...
            val = {
                'saler_name':hr_id.name or hr_id.name,
                'saler_img':hr_id.image_small or '',
    ...
    #创建任务
    @http.route('/manager/createMission', type='http', auth="none",csrf=False)
...
#获取单条拜访记录详情
    @http.route('/manager/track/<int:track_id>',type="http",auth="none",csrf=False)
...
#获取商户列表（团队负责商户，已安装，待拜访，未分配，已分配）
    @http.route('/manager/teamshop',type="http",auth="none",csrf=False)
    def teamshop(self,**post):
        ...
        if role_option=='7' or role_option=='8':
            ...
            if keyword == '':
                ...
                domain = [('business_id','in',businesses_ids),('is_sale','=',True),'|',('name','like',keyword),('street','like',keyword)]
            ...
            for shop in shops:
                val = {
                    'name' : shop.name,
                    'address' : shop.partner_id.street or '',
                    'id' : shop.id,
